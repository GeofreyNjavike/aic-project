...
class HomeController extends Controller
{
    ...
    public function index()
    {
        if (Auth::user()->clearance_level == 10) {
            $user = Player::join('users', 'players.parent_id', '=', 'users.id')
                ->get();

            $userz = DB::table('users')->where('clearance_level', '=', 5)->get();


            $count_players = Player::join('users', 'players.parent_id', '=', 'users.id')
                ->count();

            $count_payed = Player::join('users', 'players.parent_id', '=', 'users.id')
                ->where('payment', 'Payed')
                ->count();

            $count_emails = Contact::select('id')
                ->count();
            ...
            $maendeleo = Goal::join('players', 'players.player_id', '=', 'goals.player_id')
                ->get();
            ...
            $parent = Player::join('users', 'players.parent_id', '=', 'users.id')
